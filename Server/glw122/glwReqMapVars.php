...
require '../../glwdb.php';
    ...
    if($groups[1]!=""){    //Area Variations
        ...
        $datoprueba="\n".$itemsTot." / ";
        for($i=0;$i<$itemsTot;$i++){   //all areas
            ...
            if($symbols){   //symbols image
                ...
                $datoprueba.="\narea ".($i+1)." $nSymbols  $pointX1  $pointY1  $pointX2  $pointY2  $poligonSizeX   $poligonSizeY  //  ";
                if($nSymbols>2){ 
                    paintSymbols(3);
    ...
    if($groups[2]!=""){   //Circle Variations
        ...
        $diameter=0;
        for($i=0;$i<$itemsTot;$i++){   //all circles
            $circleparts=explode(":",$circles[$i]);
            $cols=explode(",",$circleparts[0]);  
            $pointX1=(integer)(($cols[2]-$pointX0)*$factor);
            $pointY1=(integer)($size_y-($cols[3]-$pointY0)*$factor);
            $diameter=(integer)($cols[7]*2*$factor);

            imagearc($image, $pointX1, $pointY1, $diameter, $diameter,  0, 360, $color);  //paint circle
...
function paintSymbols($pNumSymbols)
{
    ...
    if($pNumSymbols==1){
        ...
        imagecopy($image,$symbolRot[0],$pointX1+2+$symbolWidth0/2,$pointY1-10-$symbolHeight0,0,0,$symbolWidth0,$symbolHeight0);
        $datoprueba.="\npaintsymbols $symbolWidth0  $symbolHeight0";
        ...
        $symbolWidth2=$symbolWidth0;
        $symbolHeight2=$symbolHeight0;
        $x=$pointX1+2+$symbolWidth0/2;
        $y=$pointY1-10-$symbolHeight0;
        imagecopy($image,$symbolRot[0],$x,$y,0,0,$symbolWidth0,$symbolHeight0);
        $x=$x+2+$symbolWidth0;
        $y=$pointY1-10-$symbolHeight1;
        imagecopy($image,$symbolRot[1],$x,$y,0,0,$symbolWidth1,$symbolHeight1);
    }else if($pNumSymbols==3){
        $symbolWidth0=imagesx($symbolRot[0]);
        $symbolHeight0=imagesy($symbolRot[0]);
        $symbolWidth1=imagesx($symbolRot[1]);
        $symbolHeight1=imagesy($symbolRot[1]);
        $symbolWidth2=imagesx($symbolRot[2]);
        $symbolHeight2=imagesy($symbolRot[2]);
        $x=$pointX1+2+$symbolWidth0/2;
        $y=$pointY1-10-$symbolHeight0;
        imagecopy($image,$symbolRot[0],$x,$y,0,0,$symbolWidth0,$symbolHeight0);
        $x=$x+2+$symbolWidth0;
        ...
        imagecopy($image,$symbolRot[1],$x,$y,0,0,$symbolWidth1,$symbolHeight1);
        $x=$x+2+$symbolWidth1;
        $y=$pointY1-10-$symbolHeight2;
        imagecopy($image,$symbolRot[2],$x,$y,0,0,$symbolWidth2,$symbolHeight2);
    }
}    
