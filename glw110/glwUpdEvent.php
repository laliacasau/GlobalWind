...
if(substr($control,1,1)=="1"){
    $sqldel = "DELETE FROM glwVariations WHERE idEvents=".$idEvents." and type=1";
    $conn->query($sqldel);
    if($listas[2]!=""){
...
if(substr($control,2,1)=="1"){
    $sqldel = "DELETE FROM glwVariations WHERE idEvents=".$idEvents." and type=2)";
    $conn->query($sqldel);
    if($listas[3]!=""){
...
function wind2Boat($pWindData,$pWindVars)   //updates the base wind with local variations
{
    ...
    if($nt>0){
        ...
        if($windvar[14]=="") $res[]=$pWindData[18]; //water depth
        else $res[]=$windvar[14];
