...
//==>global variables
//configuration vars
string gsServer="http:\\globalwind.net";
...
sendCast(key pkTarget)
{
    if(giStart){ 
        if(giCast){  //is broadcast ==> stop
            ...
            giCast=0;
        }else if(giEvent){  //broadcast is stop  ==> start
...
setTouch0(integer piFace, integer piElement, key pkTarget)  //touch on panel 0 root prim
{
    if(piFace==0){   //background
        ...
        if(piElement==2){  // Send button
            if(giStart){
                if(giEvent){
                    llMessageLinked(LINK_THIS, 5001, "rdata", "2935"); //read data variations for send modif
                }else{
        ...
        if(giStart==0){
            if(gsGenKey!=""){
                ...
                llMessageLinked(LINK_THIS, 5001, "rdata", "2931"); //read data variations for start
            }
        }else{
        ...
        if(piNum>=2000 && piNum<3000){
            if(piNum>=2900){
                if(piNum==2910) setTouch0((integer)llGetSubString(psStr,0,0),(integer)llGetSubString(psStr,1,-1),pkData); //touch box  button, panel, key
                    ...
                    llMessageLinked(LINK_THIS, 5002, "rdata", "2931"); //read data variations circles
                    ...
                    llMessageLinked(LINK_THIS, 5002, "rdata", "2935"); //read data variations circles
                    ...
                    if(psStr=="1") llMessageLinked(LINK_THIS, 5001, "rdata", "2931"); //read data variations for start
